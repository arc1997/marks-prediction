...
query=("SELECT DISTINCT CAST(CLASS_NAME AS UNSIGNED INTEGER) FROM db_8_may_2018.sm_classes a,db_8_may_2018.sm_assessment_marks b WHERE a.CLASS_ID = b.CLASS_ID and STUDENT_ID='%s';")%y
...
percentages=[]
for i in a:
    query = ("SELECT CAST(b.OBTAINED_MARKS AS UNSIGNED INTEGER), CAST(c.TOTAL_MARK AS UNSIGNED INTEGER) FROM db_8_may_2018.sm_classes a,db_8_may_2018.sm_assessment_marks b , db_8_may_2018.sm_exam_subjects c WHERE a.CLASS_ID = b.CLASS_ID and b.EXAM_SUBJECT_ID = c.EXAM_SUBJECT_ID and b.STUDENT_ID='%s' and a.CLASS_NAME='%s'")%(y,i)
    mycursor = mydb.cursor()
    mycursor.execute(query)
    myresult = mycursor.fetchall()

    b=[i[0] for i in myresult]
    print(b)
